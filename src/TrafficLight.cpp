...
void TrafficLight::cycleThroughPhases(){
    ...
    auto lattesttime = std::chrono::system_clock::now();
    while (true){

		long timebetweencycles = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - lattesttime).count();

		std::this_thread::sleep_for(std::chrono::milliseconds(1));

		if (timebetweencycles >= cycleduration){
			...
			lattesttime = std::chrono::system_clock::now();
